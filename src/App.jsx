...
function saveClosedTrades(arr) {
  ...
  // 自動同步到日誌：已結束的自動推薦單轉寫為日誌條目
  try {
    const journal = JSON.parse(localStorage.getItem(TRADES_KEY) || "[]");
    arr.forEach((t) => {
      const existing = journal.find((j) => j.id === t.id);
      if (!existing && t.exitReason) {
        const pnl = t.pnlPct != null ? t.pnlPct : 0;
        const journalEntry = {
          id: t.id,
          symbol: t.symbol,
          name: t.name,
    ...
    localStorage.setItem(TRADES_KEY, JSON.stringify(journal));
  } catch {}
}
...
function RiskOverview({ longs, shorts, livePrices }) {
  ...
  let themeConcentration = {};
  all.forEach((t) => {
    keywords.forEach((kw) => {
      if ((t.name || t.symbol).toUpperCase().includes(kw)) {
        themeConcentration[kw] = (themeConcentration[kw] || 0) + 1;
      }
    });
  });
  const topTheme = Object.entries(themeConcentration).sort((a, b) => b[1] - a[1])[0];
  const themeConcentRate = topTheme ? (topTheme[1] / total) * 100 : 0;
  // 平均未實現盈虧
